...
void Sub_worlds::reset(const Cell_group &world, const Cell_group &bridges, const Connections &connections) {
    gates.clear();
    _sub_worlds.clear();
            ...
            for ( uint32_t j = last_completed; j < sub_world.cells.size() && cell_id == Not_found; j++ ){
                ...
                for( uint32_t k = 0; k < c.size() && cell_id == Not_found; k++ ){
                    ...
                    if (_cell_sub_world_index[cnn.id] == Not_found ) {
                        ...
                        sub_world.gate_cells.add(world[cnn.id]);
                    }
                }
...
Cell_group Sub_worlds::find_bridges(const Cell_group &world, const Connections &connections) {
    Cell_group candidates;
    // step 1 add all cells with at least 2 connections and less than 5
    for (uint32_t i = 0 ; i < world.size(); i++ ) {
        if (connections[i].connections.size()>=3 && connections[i].connections.size() <=4) candidates.add(world[i]);
    }
    ...
    for (uint32_t i = 0; i < candidates.size(); i++) {
        ...
        if (!gate_by_cell_id(cell_id).gate_connections.empty()) candidates2.add(world[cell_id]);
    }
    ...
    for (uint32_t i = 0; i < candidates2.size(); i++) {
        ...
        if (_sub_worlds.size() < target_size) candidates3.add(world[cell_id]);
    }
    ...
    for (uint32_t i = 0; i < candidates3.size(); i++) {
        ...
        if (gate_by_cell_id(cell_id).gate_connections.size()==1) candidates4.remove(world[cell_id]);
    }
    return candidates4;
}
...
Sub_worlds::Sub_worlds(const Cell_group &world, const Cell_group &bridges, const Connections &connections) {
