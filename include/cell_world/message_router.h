...
#define Add_route(HEADER, DESTINATION, ...) { if (message.header == HEADER) { routed = true; try { DESTINATION(__VA_OPT__(message.get_body<__VA_ARGS__>())); } catch (...) { failed_route(message); } }}
